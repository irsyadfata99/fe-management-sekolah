...
export default function ArticlesPage() {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      // ✅ FIXED: backend expect 'kategori_id' not 'kategori'
      ...
      const response = await fetch(
        `http://localhost:5000/api/admin/articles?${params}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleTogglePublish = async (id: number) => {
    try {
      const token = localStorage.getItem("token");
      // ✅ FIXED: use PATCH method instead of POST
      const response = await fetch(
        `http://localhost:5000/api/admin/articles/${id}/publish`,
        {
          method: "PATCH",
          headers: {
  ...
  const handleToggleFeatured = async (id: number) => {
    try {
      ...
      // ✅ FIXED: use PATCH method instead of POST
      const response = await fetch(
        `http://localhost:5000/api/admin/articles/${id}/feature`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://localhost:5000/api/admin/articles/${deleteModal.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Kelola artikel dan konten website sekolah
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          ...
          <p className="text-2xl font-bold text-gray-800">
            {articles.filter((a) => a.is_published === 1).length}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-yellow-500">
          <p className="text-sm text-gray-600">Draft</p>
          <p className="text-2xl font-bold text-gray-800">
            {articles.filter((a) => a.is_published === 0).length}
          </p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
          <p className="text-sm text-gray-600">Featured</p>
          <p className="text-2xl font-bold text-gray-800">
            {articles.filter((a) => a.is_featured === 1).length}
          </p>
        </div>
      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        {article.gambar_utama && (
                          <Image
                            src={`http://localhost:5000/uploads/articles/${article.gambar_utama}`}
                            alt={article.judul}
                            width={64}
                            height={64}
                        ...
                        <div>
                          ...
                          <div className="text-sm text-gray-500">
                            {article.konten_singkat?.substring(0, 60)}...
                          </div>
                        </div>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">
              ...
              <span className="font-semibold">{deleteModal.judul}</span>&quot;?
